...
class Urllib2Client(HTTPClient):
    ...
    def __init__(self, verify_ssl_certs=True, proxy=None):
        ...
        self._opener = None
        if self._proxy:
            proxy = urllib.session.ProxyHandler(self._proxy)
            self._opener = urllib.session.build_opener(proxy)
    ...
    def _request_internal(self, method, url, headers, post_data, is_streaming):
        ...
        req = urllib.session.Request(url, post_data, headers)
        ...
        try:
            ...
            response = (
                ...
                if self._opener
                else urllib.session.urlopen(req)
            )
